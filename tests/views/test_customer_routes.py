...
class TestCustomerRoutes(BaseTestCase):
    ...
    if not customer_data:
        customer_data = {"phone_number": str(phone_number)}

    token_data = {"id": "991d6ed53abc4fa4b579a2abcab4bbc7", "token": "893470"}
    customer_info = {
    ...
    def test_l_reset_pin(self):
        ...
        with self.client:
            response = self.client.post(
                "/api/v1/customers/reset-pin/{}".format(self.user_data.get("id")),
                json={"password_token": self.user_data["password_token"], "pin": "6666"},
            )
            self.assert_status(response, 200)
